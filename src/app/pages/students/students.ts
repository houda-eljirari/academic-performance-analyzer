...
import { LucideAngularModule,
         ...
         GraduationCap, Users, Filter } from 'lucide-angular';
        ...
        if (results && results.length > 0) {
          this.students = results.map((s: any) => {
            ...
            return {
              ...
              email:       `${s.id_student}@oulad.ac.uk`,
              initials:    s.gender === 'M' ? 'M' : 'F',
              filiere:     s.region || 'OULAD',
              average:     0,
              status:      result === 'Pass'        ? 'Admis'  :
                           result === 'Distinction' ? 'Admis'  :
                           result === 'Fail'        ? 'Echec'  : 'Risque',
              statusClass: result === 'Pass'        ? 'success' :
                           result === 'Distinction' ? 'success' :
                           result === 'Fail'        ? 'danger'  : 'warning',
            };
          });
        }
      },
      error: () => console.log('API indisponible — données statiques utilisées'),
    });
  }
